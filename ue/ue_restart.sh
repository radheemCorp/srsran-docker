...
case "$TARGET" in
  all)
    ...
    exec_in_service "$UE1_DIR" ue1 "pkill -f srsue || true" || true
    exec_in_service "$UE2_DIR" ue2 "pkill -f srsue || true" || true
...
echo "  docker compose --project-directory \"$UE1_DIR\" -f \"$UE1_DIR/docker-compose.yaml\" exec -it ue1 bash -lc '/srsran/config/start_ue.sh 1'"
echo "  docker compose --project-directory \"$UE2_DIR\" -f \"$UE2_DIR/docker-compose.yaml\" exec -it ue2 bash -lc '/srsran/config/start_ue.sh 2'"
